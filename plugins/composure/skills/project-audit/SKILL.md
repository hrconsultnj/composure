...
4. Create output directory: `mkdir -p tasks-plans/audits`
5. Capture timestamp: `date +"%Y-%m-%d-%H%M"`
...
Generate the HTML following this structure. Replace all `{PLACEHOLDER}` values with collected data. Omit entire tab buttons and panels for unavailable plugins. Use inline `style="color:{HEX}"` for grade colors (not CSS variables) so print mode works correctly.

Grade color mapping: A = `#22c55e`, B = `#3b82f6`, C = `#eab308`, D = `#f97316`, F = `#ef4444`.
...
<head>
  ...
  <title>{PROJECT} — Project Audit Report</title>
  <style>/* PASTE COMPLETE CSS FROM ABOVE VERBATIM */</style>
</head>
<body>

  <div class="report-header">
    <div class="brand">Project Audit</div>
    <h1>{PROJECT_NAME}</h1>
    <div class="meta">{DATE} &middot; {TOTAL_FILES} files &middot; {TOTAL_LINES} lines</div>
    <div class="grade-circle" style="color: {OVERALL_GRADE_COLOR}">
      <span class="letter" style="color: {OVERALL_GRADE_COLOR}">{OVERALL_GRADE}</span>
      <span class="score-num">{OVERALL_SCORE}/100</span>
    </div>
  </div>

  <div class="container">
    <!-- Executive Summary: .score-grid with one .score-card per available category -->
    <!-- Tab Bar: .tab-bar with one .tab-btn per available category + Stack -->
    <!-- Tab Panels: one .tab-content per tab -->
    <!-- Recommendations: .rec-list after all tabs -->
  </div>

  <div class="report-footer">
    <p>Generated by <strong>Composure</strong> on {DATE}</p>
    <p><a href="https://github.com/hrconsultnj/claude-plugins">github.com/hrconsultnj/claude-plugins</a></p>
    <a class="cta" href="https://buymeacoffee.com/hrconsultnj">Get a Professional Review &mdash; Project Audit ($99)</a>
  </div>

  <script>/* PASTE COMPLETE JS FROM ABOVE VERBATIM */</script>
</body>
</html>
```
