...
export function checkQuality(filePath: string): QualityReport | null {
  ...
  if (lineCount < 100) return report;
  ...
  if (!basename.startsWith("types.")) {
    ...
    if (report.inlineTypeCount > 3) {
      report.suggestions.push(
        `Move ${report.inlineTypeCount} inline types (${report.inlineTypes.join(", ")}) to types.ts`,
      );
    }
  }
  ...
  if (basename.endsWith(".tsx")) {
    ...
    if (components.length > 2) {
      report.suggestions.push(
        `Split ${components.length} exported components (${components.join(", ")}) into separate files`,
      );
    }
  }
  ...
  for (const fn of report.largeFunctions) {
    report.suggestions.push(
      `Extract \`${fn.name}\` (lines ${fn.lineStart}-${fn.lineEnd}, ~${fn.lines} lines)`,
    );
  }

  // ── Determine severity ──────────────────────────────────────
  const hasViolation =
    report.inlineTypeCount > 3 ||
