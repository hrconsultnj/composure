 ...
 */
export function runFrameworkValidation(
  filePath: string,
  relativePath: string,
  content: string,
  config: ComposureConfig,
  pluginRoot: string | null,
): { violations: Violation[]; warnings: Violation[] } {
  ...
  if (pluginRoot) {
    ...
    if (config.frameworks) {
      ...
      for (const fw of Object.values(config.frameworks)) {
        if (fw.frontend) frontends.add(fw.frontend);
        if (fw.backend) backends.add(fw.backend);
      }
      ...
      if (frontends.size > 0) {
        ...
        if (existsSync(frontendDir)) {
          for (const f of readdirSync(frontendDir).filter((f) =>
            f.endsWith(".json"),
          )) {
  ...
  if (config.frameworkValidation) {
    for (const [groupName, group] of Object.entries(
      config.frameworkValidation,
    )) {
      // Skip if plugin already defined this group
      if (pluginGroupNames.has(groupName)) continue;
  ...
  if (config.frameworks) {
    const hasNextjs = Object.values(config.frameworks).some(
      (fw) => fw.frontend === "nextjs",
    );
    if (hasNextjs) {
      const basename = filePath.split("/").pop() ?? "";
      if (
        /^(app|src\/app)\/.*\.tsx$/.test(relativePath) &&
        ![
  ...
  function processRuleFile(dir: string, filename: string) {
  ...
  function evaluateGroup(groupName: string, group: FrameworkRuleGroup) {
    if (!matchesGlobs(relativePath, group.appliesTo)) return;
    ...
    for (const rule of group.rules) {
      ...
      // Check skipIf
      if (rule.skipIf && new RegExp(rule.skipIf).test(content)) continue;
