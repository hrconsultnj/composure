...
export function isPkgParseable(filePath: string): boolean {
...
export function parsePkgFile(filePath: string): { nodes: NodeInfo[]; edges: EdgeInfo[] } {
  let content: string;
  try {
...
function parseTurboJson(
  ...
  let turbo: { tasks?: Record<string, unknown>; pipeline?: Record<string, unknown> };
  try {
  ...
  for (const taskName of Object.keys(tasks)) {
    const taskConfig = tasks[taskName] as Record<string, unknown>;
    ...
    const dependsOn = (taskConfig?.dependsOn ?? []) as string[];
...
function findJsonLine(content: string, needle: string): number {
...
function findYamlLine(content: string, needle: string): number {
