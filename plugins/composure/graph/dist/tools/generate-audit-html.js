...
function findTemplateDir() {
    ...
    for (const dir of candidates) {
        if (existsSync(join(dir, "audit-header.html")))
            return dir;
    }
    return null;
}
...
function buildReplacementMap(store, runId, repoRoot) {
    ...
    const securityRows = findings
        ...
        .slice(0, 10)
        .map((f) => `<tr><td><span class="badge badge-${f.severity}">${f.severity}</span></td><td>${escHtml(f.title)}</td><td class="path">${escHtml(f.file_path ?? "dependency")}</td></tr>`)
        .join("\n");
    // Build untested function rows
    const untestedRows = gaps
        .slice(0, 15)
        .map((g) => `<tr><td class="path">${escHtml(g.file_path)}</td><td>${escHtml(g.node_qualified_name.split("::").pop() ?? "")}</td></tr>`)
        .join("\n");
...
//# sourceMappingURL=generate-audit-html.js.map