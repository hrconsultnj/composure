...
// ── Detection ─────────────────────────────────────────────────────
export function isYamlParseable(filePath) {
...
function extractReferences(doc) {
    const refs = [];
    const seen = new Set();
    function addRef(type, name, match) {
        const key = `${type}:${name}`;
        if (!seen.has(key)) {
            seen.add(key);
            ...
            if (type === "configMap")
                refs[refs.length - 1].target = `ConfigMap/${name}`;
            else if (type === "secret")
                refs[refs.length - 1].target = `Secret/${name}`;
            else if (type === "serviceAccount")
                refs[refs.length - 1].target = `ServiceAccount/${name}`;
            else if (type === "pvc")
                refs[refs.length - 1].target = `PersistentVolumeClaim/${name}`;
        }
    }
