...
import type { PromptFormat } from "./engine/prompt-generator.js";
...
const commands: Record<string, Handler> = {
  evaluate: (args) => {
    const ruleset = args.ruleset_json
      ? JSON.parse(args.ruleset_json as string)
      : loadRuleset((args.ruleset as string) ?? "general");
    if (!ruleset) return { status: "error", error: `Ruleset not found`, available: AVAILABLE };
    return { status: "ok", ...evaluate({ response: args.response as string, user_input: args.user_input as string, ruleset }) };
  },
  ...
  prompt: (args) => {
    const ruleset = args.ruleset_json
      ? JSON.parse(args.ruleset_json as string)
      : loadRuleset((args.ruleset as string) ?? "general");
    if (!ruleset) return { status: "error", error: `Ruleset not found` };
    return { status: "ok", prompt: generateSystemPrompt(ruleset, (args.format as PromptFormat) ?? "generic") };
  },
...
if (process.argv[3]) {
  try { args = JSON.parse(process.argv[3]); } catch {
