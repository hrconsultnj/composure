...
function loadRuleset(name: string): Ruleset | null {
  try {
...
server.tool(
  "evaluate_response",
  `Evaluate an AI agent's response against a guardrails ruleset. Runs 5 decision trees in <10ms:
...
Returns: verdict (pass/redirect/modify/block), reason, suggestion, full decision path.`,
  ...
  async ({ response, user_input, ruleset_name, ruleset_json }) => {
    ...
    if (ruleset_json) {
      try { ruleset = JSON.parse(ruleset_json); } catch {
        return { content: [{ type: "text" as const, text: JSON.stringify({ status: "error", error: "Invalid ruleset_json" }) }] };
      }
    } else {
    ...
    if (!ruleset) {
      return { content: [{ type: "text" as const, text: JSON.stringify({ status: "error", error: `Ruleset not found: ${ruleset_name}`, available: AVAILABLE_RULESETS }) }] };
    }

    const result = evaluate({ response, user_input, ruleset });
    return { content: [{ type: "text" as const, text: JSON.stringify({ status: "ok", ...result }, null, 2) }] };
  },
);

server.tool(
  "generate_prompt",
  "Generate optimized system prompt sections from a guardrails ruleset. Zero latency — guardrails are 'baked in' to the prompt. Supports platform-specific formats.",
  {
  ...
  async ({ ruleset_name, ruleset_json, format }) => {
    ...
    if (!ruleset) {
      return { content: [{ type: "text" as const, text: JSON.stringify({ status: "error", error: `Ruleset not found: ${ruleset_name}` }) }] };
    }

    const prompt = generateSystemPrompt(ruleset, format ?? "generic");
    return { content: [{ type: "text" as const, text: JSON.stringify({ status: "ok", format: format ?? "generic", prompt }, null, 2) }] };
  },
);

server.tool(
  "load_ruleset",
  "Load a pre-built guardrails ruleset by name. Returns the full ruleset JSON for inspection or modification.",
  {
  ...
  async ({ name }) => {
    ...
    if (!ruleset) {
      return { content: [{ type: "text" as const, text: JSON.stringify({ status: "error", error: `Ruleset not found: ${name}` }) }] };
    }
    return { content: [{ type: "text" as const, text: JSON.stringify({ status: "ok", ruleset }, null, 2) }] };
  },
);

server.tool(
  "list_rulesets",
  "List all available pre-built guardrails rulesets.",
  {},
  async () => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ status: "ok", rulesets }, null, 2) }] };
  },
);
