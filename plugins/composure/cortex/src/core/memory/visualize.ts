...
import type { ToolResult, MemoryNode, MemoryEdge } from "../types.js";
import type { StorageAdapter } from "../../adapters/types.js";

export async function generateMemoryHtml(
  adapter: StorageAdapter,
  params: { agent_id: string; output_path?: string }
): Promise<ToolResult> {
  try {
    ...
    const nodesJson = JSON.stringify(
      nodesResult.map((n) => ({
        ...
        content: n.content,
        type: (n.metadata as any)?.category ?? "text",
        color: typeColors[(n.metadata as any)?.category] ?? typeColors.text,
        metadata: n.metadata,
...
simulation.on('tick', () => {
  ...
  node.attr('transform', d => 'translate(' + d.x + ',' + d.y + ')');
    ...
    if (params.output_path) {
      ...
      try { mkdirSync(dirname(params.output_path), { recursive: true }); } catch {}
