...
import type { ToolResult, ThinkingParams, ThoughtType } from "../types.js";
  ...
  try {
    ...
    if (params.is_revision && params.revises_thought != null) {
      const targetExists = steps.some(
        (s) =>
          s.thought_number === params.revises_thought &&
    ...
    if (params.branch_id && params.branch_from_thought != null) {
      const branchPointExists = steps.some(
        (s) => s.thought_number === params.branch_from_thought && !s.branch_id
      );
    ...
    const thoughtType: ThoughtType = params.is_revision
      ? "revision"
