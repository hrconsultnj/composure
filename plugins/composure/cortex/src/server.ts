...
server.tool(
  ...
  "Retrieve a thinking session with all its steps, ordered by branch and thought number.",
  {
...
server.tool(
  ...
  "List thinking sessions for an agent, optionally filtered by status.",
  {
...
server.tool(
  ...
  "Mark a thinking session as completed with an optional conclusion.",
  {
...
server.tool(
  "summarize_thinking_session",
  "Get a structured summary of a thinking session — thought chains, branches, revisions, and conclusion.",
  {
...
server.tool(
  "capture_thought",
  "Persist an externally-initiated reasoning step into Cortex with full context and entity_registry linkage. Used by the seq-thinking reflex hook.",
  {
...
server.tool(
  "search_memory",
  "Search the memory knowledge graph using full-text search with optional filtering by tags, category, and time range.",
  {
...
server.tool(
  ...
  "Semantic search using vector embeddings with optional keyword boost (70% semantic + 30% keyword). Requires a pre-computed embedding vector.",
  {
    agent_id: z.string().describe("Agent ID to search within."),
...
server.tool(
  ...
  "Walk the memory graph from a starting node, collecting connected nodes within a given depth via BFS.",
  {
