...
/**
 * Generates an id_prefix matching the cortex parity format.
 *
 * Format: UPPERCASE_PREFIX + 12 lowercase alphanumeric chars (a-z, 0-9)
 * Example: USRa1b2c3d4e5f6, AMN0a6ullh21zty
 *
 * Uses Web Crypto's getRandomValues via Uint32Array (cryptographically
 * secure source, modulo bias < 10^-8 — negligible for id purposes).
 *
 * Supported prefixes:
...
export class SqliteAdapter implements StorageAdapter {
  ...
  private initSchema(): void {
    this.db.exec(`
      ...
      -- tasks-plans/reference/cortex-schema-postgres-reference-2026-04-10.md §4.3.
    ...
    // ── Phase S1 version marker (schema v2) ─────────────────────
    // The parity tables (accounts/users/ai_agents/projects/tasks/
    // cortex_sessions) are already created above via CREATE TABLE
    // IF NOT EXISTS. This gate just stamps the schema version so
    // Phase S2 can detect post-S1 state and run its ALTERs against
    // existing ai_memory_nodes / ai_thinking_sessions cleanly.
    if (currentVersion < 2) {
