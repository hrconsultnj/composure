...
// dist/core/thinking/templates/index.js
var templates = {
  "code-review": {
    id: "code-review",
    name: "Code Review",
...
function getNextStep(templateId, currentThoughtNumber) {
...
// dist/core/thinking/session.js
async function createSession(adapter, params) {
  try {
    let template = null;
    ...
    const title = params.title ?? template?.name ?? void 0;
...
async function resumeSession(adapter, params) {
  try {
...
async function completeSession(adapter, params) {
  try {
...
async function listSessions(adapter, params) {
  try {
...
// dist/core/thinking/step.js
async function addThought(adapter, params) {
  try {
    ...
    if (!sessionId) {
      ...
      const session2 = await adapter.createSession(params.agent_id, params.title);
    ...
    if (params.is_revision && params.revises_thought != null) {
      const targetExists = steps.some((s) => s.thought_number === params.revises_thought && (params.branch_id ? s.branch_id === params.branch_id : !s.branch_id));
      if (!targetExists) {
        return {
          status: "error",
    ...
    const thoughtType = params.is_revision ? "revision" : params.branch_id && params.branch_from_thought != null ? "branch" : params.thought_type ?? "analysis";
    const step = await adapter.addStep(sessionId, {
      ...
      needs_more_thoughts: params.total_thoughts != null ? thoughtNumber < params.total_thoughts : true,
      metadata: {}
    });
    const newTotal = session.total_thoughts + 1;
    await adapter.updateSession(sessionId, { total_thoughts: newTotal });
    const needsMore = params.total_thoughts != null ? thoughtNumber < params.total_thoughts : true;
    return {
      status: "ok",
...
// dist/core/thinking/summarize.js
async function summarizeSession(adapter, params) {
  try {
    const result = await adapter.getSession(params.session_id);
    if (!result) {
      return { status: "not_found", error: `Session ${params.session_id} not found` };
    ...
    const conclusionStep = steps.filter((s) => s.thought_type === "conclusion").pop();
    const summary = {
      session_id: session.id,
      title: session.title,
    ...
    return {
      status: "ok",
      summary,
      thought_chain: steps.filter((s) => !s.branch_id).map((s) => ({
        number: s.thought_number,
        type: s.thought_type,
        thought: s.thought,
        is_revision: s.is_revision
      })),
      branch_chains: Object.fromEntries(branches.map((branch) => [
        branch,
        steps.filter((s) => s.branch_id === branch).map((s) => ({
          number: s.thought_number,
          type: s.thought_type,
          thought: s.thought
        }))
...
__export(memory_exports, {
  ...
  generateMemoryHtml: () => generateMemoryHtml,
...
// dist/core/memory/entities.js
async function createNode(adapter, params) {
  try {
...
async function getNode(adapter, params) {
  try {
...
async function updateNode(adapter, params) {
  try {
...
async function deleteNode(adapter, params) {
  try {
...
// dist/core/memory/relations.js
async function createEdge(adapter, params) {
  try {
...
async function getEdgesForNode(adapter, params) {
  try {
...
async function deleteEdge(adapter, params) {
  try {
...
// dist/core/memory/search.js
async function searchMemory(adapter, params) {
  try {
...
async function searchSemantic(adapter, params) {
  try {
...
async function searchWithText(adapter, params) {
...
// dist/core/memory/traverse.js
async function traverseGraph(adapter, params) {
...
// dist/core/memory/visualize.js
async function generateMemoryHtml(adapter, params) {
  try {
...
document.getElementById('stats').textContent =
  nodes.length + ' nodes \xB7 ' + links.length + ' edges \u2014 Composure Cortex Memory Graph';
...
async function createGraphLink(adapter, params) {
...
async function searchByGraphEntity(adapter, params) {
...
async function deleteGraphLinksForNode(adapter, params) {
  try {
...
var SupabaseAdapter = class {
  ...
  async getSession(session_id) {
    const { data: session, error: sErr } = await this.client.from("ai_thinking_sessions").select().eq("id", session_id).single();
    if (sErr || !session)
      return null;
    const { data: steps, error: stErr } = await this.client.from("ai_thinking_steps").select().eq("session_id", session_id).order("thought_number", { ascending: true });
    if (stErr)
  ...
  async listSessions(agent_id, status) {
    let query = this.client.from("ai_thinking_sessions").select().eq("agent_id", agent_id).order("created_at", { ascending: false });
    if (status) {
  ...
  async updateSession(session_id, updates) {
    const { data, error } = await this.client.from("ai_thinking_sessions").update(updates).eq("id", session_id).select().single();
    if (error)
  ...
  async addStep(session_id, step) {
    const { data, error } = await this.client.from("ai_thinking_steps").insert({ session_id, ...step }).select().single();
    if (error)
      throw new Error(`addStep: ${error.message}`);
    return data;
  }
  async getSteps(session_id, branch_id) {
    let query = this.client.from("ai_thinking_steps").select().eq("session_id", session_id).order("thought_number", { ascending: true });
    if (branch_id) {
  ...
  async createNode(params) {
    const { data, error } = await this.client.from("ai_memory_nodes").insert({
      agent_id: params.agent_id,
  ...
  async getNode(node_id) {
    const { data, error } = await this.client.from("ai_memory_nodes").select().eq("id", node_id).single();
    if (error)
      return null;
    return data;
  }
  async updateNode(node_id, params) {
    const { data, error } = await this.client.from("ai_memory_nodes").update(params).eq("id", node_id).select().single();
    if (error)
      throw new Error(`updateNode: ${error.message}`);
    return data;
  }
  async deleteNode(node_id) {
    const { error, count } = await this.client.from("ai_memory_nodes").delete().eq("id", node_id);
    if (error)
  ...
  async createEdge(params) {
    const { data, error } = await this.client.from("ai_memory_edges").insert({
      agent_id: params.agent_id,
  ...
  async getEdgesForNode(node_id) {
    const { data, error } = await this.client.from("ai_memory_edges").select().or(`from_node_id.eq.${node_id},to_node_id.eq.${node_id}`);
    if (error)
      throw new Error(`getEdgesForNode: ${error.message}`);
    return data ?? [];
  }
  async deleteEdge(edge_id) {
    const { error, count } = await this.client.from("ai_memory_edges").delete().eq("id", edge_id);
    if (error)
  ...
  async traverseGraph(start_node_id, depth, relationship_types) {
    const visitedNodeIds = /* @__PURE__ */ new Set();
    const allEdges = [];
    ...
    const { data: nodes, error } = await this.client.from("ai_memory_nodes").select().in("id", nodeIds);
    if (error)
  ...
  async createGraphLink(params) {
    ...
    const { data, error } = await this.client.from("ai_graph_links").insert({
      id,
    ...
    if (error)
      throw new Error(`createGraphLink: ${error.message}`);
    return data;
  }
  async searchByGraphEntity(params) {
    ...
    if (error)
      throw new Error(`searchByGraphEntity: ${error.message}`);
  ...
  async deleteGraphLinksForNode(memory_node_id) {
    const { data, error } = await this.client.from("ai_graph_links").delete().eq("memory_node_id", memory_node_id).select();
    if (error)
      throw new Error(`deleteGraphLinksForNode: ${error.message}`);
...
var SqliteAdapter = class _SqliteAdapter {
  ...
  async listSessions(agent_id, status) {
    const sql = status ? "SELECT * FROM ai_thinking_sessions WHERE agent_id = ? AND status = ? ORDER BY created_at DESC" : "SELECT * FROM ai_thinking_sessions WHERE agent_id = ? ORDER BY created_at DESC";
    const rows = status ? this.db.prepare(sql).all(agent_id, status) : this.db.prepare(sql).all(agent_id);
  ...
  async updateSession(session_id, updates) {
    ...
    if (updates.total_thoughts !== void 0) {
  ...
  async getSteps(session_id, branch_id) {
    const sql = branch_id ? "SELECT * FROM ai_thinking_steps WHERE session_id = ? AND branch_id = ? ORDER BY thought_number" : "SELECT * FROM ai_thinking_steps WHERE session_id = ? ORDER BY thought_number";
    const rows = branch_id ? this.db.prepare(sql).all(session_id, branch_id) : this.db.prepare(sql).all(session_id);
  ...
  async createEdge(params) {
    ...
    this.db.prepare(`INSERT INTO ai_memory_edges (id, agent_id, from_node_id, to_node_id, relationship_type, weight, metadata, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, params.agent_id, params.from_node_id, params.to_node_id, params.relationship_type, params.weight ?? 1, JSON.stringify(params.metadata ?? {}), ts);
    return { id, agent_id: params.agent_id, from_node_id: params.from_node_id, to_node_id: params.to_node_id, relationship_type: params.relationship_type, weight: params.weight ?? 1, metadata: params.metadata ?? {}, created_at: ts };
  }
  async getEdgesForNode(node_id) {
...
// dist/adapters/sync.js
async function syncUp(local, remote, agentId) {
  const result = {
    direction: "up",
    sessions_synced: 0,
    steps_synced: 0,
  ...
  try {
    ...
    for (const session of localSessions) {
      try {
        ...
        if (!remoteSession) {
          const created = await remote.createSession(agentId, session.title ?? void 0);
    ...
    const localNodes = await local.searchMemory({ agent_id: agentId, limit: 1e3 });
...
async function syncDown(local, remote, agentId) {
  const result = {
    direction: "down",
    sessions_synced: 0,
    steps_synced: 0,
  ...
  try {
    ...
    for (const session of remoteSessions) {
      try {
        ...
        if (!localSession) {
          const created = await local.createSession(agentId, session.title ?? void 0);
    ...
    const remoteNodes = await remote.searchMemory({ agent_id: agentId, limit: 1e3 });
...
async function syncStatus(local, remote, agentId) {
  try {
    const localSessions = await local.listSessions(agentId);
    const remoteSessions = await remote.listSessions(agentId);
...
var commands = {
  // Thinking
  sequential_think: (a, args) => thinking_exports.addThought(a, args),
  get_thinking_session: (a, args) => thinking_exports.resumeSession(a, args),
  list_thinking_sessions: (a, args) => thinking_exports.listSessions(a, args),
  ...
  update_memory_node: (a, args) => memory_exports.updateNode(a, args),
  delete_memory_node: (a, args) => memory_exports.deleteNode(a, args),
  create_memory_edge: (a, args) => memory_exports.createEdge(a, args),
  get_edges_for_node: (a, args) => memory_exports.getEdgesForNode(a, args),
  delete_memory_edge: (a, args) => memory_exports.deleteEdge(a, args),
  search_memory: (a, args) => memory_exports.searchMemory(a, args),
  search_memory_semantic: (a, args) => memory_exports.searchSemantic(a, args),
  search_memory_text: (a, args) => memory_exports.searchWithText(a, args),
  traverse_memory_graph: (a, args) => memory_exports.traverseGraph(a, args),
  generate_memory_html: (a, args) => memory_exports.generateMemoryHtml(a, args),
  // Graph ↔ Memory Links
  create_graph_link: (a, args) => memory_exports.createGraphLink(a, args),
  search_by_graph_entity: (a, args) => memory_exports.searchByGraphEntity(a, args),
  delete_graph_links_for_node: (a, args) => memory_exports.deleteGraphLinksForNode(a, args),
  // Sync
