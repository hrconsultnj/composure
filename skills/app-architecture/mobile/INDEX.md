...
Mobile frameworks still use frontend patterns. **You MUST also load `frontend/INDEX.md`** and follow its "Always Load" instructions — read `core.md` (routing file), then ALL files in `references/typescript/` and `references/generated/`.

## Load by `frontend` value

| `frontend` value | MUST load | Contains |
|---|---|---|
| `"expo"` | [expo/expo.md](expo/expo.md) + ALL files in [expo/references/](expo/references/) | Phase 5 (overlay guards), Phase 7 (expo-router), anti-patterns (icons, bottom sheets, native modules, config plugins) |
