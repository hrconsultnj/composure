...
Standard shadcn components cover most UI needs. For anything beyond — landing pages, hero sections, dashboards with visual flair, interactive experiences, creative portfolios — the **Design Forge** plugin provides production-ready patterns:

- **When to use:** The user asks for animations, generative backgrounds, glassmorphism, custom cursors, scroll effects, 3D elements, particle systems, or "make it look premium"
- **How:** Invoke `/design-forge` to browse patterns, or ask about a specific component (e.g., "add a generative canvas background")
- **Research first:** Use `/ux-researcher` to discover design patterns and analyze competitors before building

See `frontend/INDEX.md` → "Design & Visual Quality" for the full component catalog and install instructions.
